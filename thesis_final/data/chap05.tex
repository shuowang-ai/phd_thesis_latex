...
气象再分析数据（$\mathbf{M}$）：气象驱动$\mathbf{M} \in \mathbb{R}^{N \times D_M}$（$D_M=7$）同样采用ERA5再分析数据集\citep{hersbach2020era5}（详见第\ref{subsec:meteo_data}节），选取气温、风速风向、降水、短波辐射、相对湿度和地表气压等变量，空间分辨率为0.25°×0.25°，时间分辨率为3小时。

排放清单数据（$\mathbf{E}$）：排放驱动$\mathbf{E} \in \mathbb{R}^{N \times D_E}$（$D_E=5$）采用MEIC清单\citep{zheng2018trends}（详见第\ref{subsec:emis_data}节），包含\PM、\PMten、\SOtwo、\NOx 和\VOC 等物种，原始月度数据通过时间降尺度转换为3小时分辨率。
...
\begin{table}[htbp]
    ...
    \begin{tabular}{@{}lllccc@{}}
        ...
        \midrule
        \multirow{2}{*}{污染物（$\mathbf{X}$）} & \PM & $X_{\text{PM}_{2.5}}$ & $\mu$g m$^{-3}$ & 3小时 & CNEMC \\
        & \ozone & $X_{\text{O}_3}$ & $\mu$g m$^{-3}$ & 3小时 & CNEMC \\
        \midrule
...
\noindent 式中$\mathbf{M}_{t-T'+1:t}$表示过去$T'$个时间步的气象信息（包含温度、风速、风向、降水、辐射、湿度、气压7个变量），$\mathbf{E}_{t-T'+1:t}$表示对应时段的排放数据（包含\PM、\PMten、\SOtwo、\NOx、\VOC 共5个物种）；输出$\hat{\mathbf{X}}_t \in \mathbb{R}^{N \times 2}$为时刻$t$的\PM 和\ozone 浓度。该设计避免了将历史污染物浓度作为输入所带来的误差累积问题\citep{qi2019hybrid}。
...
然而，与\PM 的下降趋势相反，年平均\ozone 浓度在除基准情景外的所有策略下都呈现出显著的上升趋势，AAC在$+0.07$至$+0.22$ $\mu$g m$^{-3}$之间（$p < 0.05$）。这一结果预示着，未来\ozone 污染可能会取代\PM，成为该区域面临的主要大气环境挑战。

\begin{figure}[htbp]
  \centering
  \includegraphics[width=\linewidth]{figures/chap05_future_average.pdf}
  \caption{不同排放策略下\cqt{2+26}城市未来PM$_{2.5}$与O$_3$浓度变化趋势}
...
\begin{figure}[htbp]
  ...
  \caption{当前目标策略下六城市未来PM$_{2.5}$与O$_3$浓度变化趋势}
...
在\ozone 变化方面，值得注意的是，北京和太原的\ozone 浓度呈现微弱的下降趋势，这与区域总体上升趋势形成对比。一种可能的解释是，北京和太原均处于VOC控制区（VOC-limited regime）\citep{li2019two}：在此类光化学环境中，\ozone 生成主要受VOC浓度制约，\NOx 减排反而会通过减弱\ozone 的化学滴定效应（$\mathrm{NO}$ + \ozone $\rightarrow$ \NOtwo + $\mathrm{O}_2$）导致\ozone 浓度上升。而随着减排策略持续推进，当\NOx 浓度降至足够低时，光化学体系逐步向\NOx 控制区过渡，\ozone 生成速率随\NOx 减少而降低，从而呈现出与其他VOC控制区城市不同的下降趋势。这一差异化响应表明，不同城市的最优减排路径取决于其所处的光化学敏感性区间，需要因地制宜地制定协同减排策略。
...
\begin{table}[htbp]
    ...
    \begin{tabular}{@{}lc|c@{}}
        \toprule
        \textbf{排放策略} & \textbf{\PM重污染日 AAC} & \textbf{\ozone重污染日 AAC} \\
        \midrule
...
（2）\textbf{揭示了\PM 与\ozone 的反向演变趋势与协同治理需求。}在所有排放策略下，\cqt{2+26}城市未来\PM 浓度将持续下降（年均$-0.14$至$-0.37$ $\mu$g m$^{-3}$），而\ozone 浓度普遍上升（年均$+0.07$至$+0.22$ $\mu$g m$^{-3}$）。这一\cqt{气候惩罚}效应表明，由于\PM 和\ozone 共享\NOx 等前驱物且生成机制相互影响，单一污染物控制策略可能引发负面效果，必须根据区域化学敏感性科学确定协同减排比例。该发现也与国家\cqt{双碳}目标高度契合——能源结构转型在减少\COtwo 排放的同时将天然带来空气质量改善的协同效益。

（3）\textbf{量化了不同城市的差异化响应。}北京和太原的\ozone 浓度呈微弱下降趋势，可能与其处于VOC控制区有关\citep{li2019two}，为因地制宜制定减排策略提供了科学依据。在三种最严格的减排策略下，\PM 重污染事件预计将在2030年前后被基本消除。

本章研究仍存在局限：所选变量可能未涵盖所有关键大气过程，IGNN作为数据驱动方法在捕捉物理机制方面存在不足。此外，不同城市间\ozone 趋势的差异化响应（如北京和太原的下降趋势）尚需更深入的机理解释，未来可引入归因分析（Attribution Analysis）方法，结合光化学敏感性指标量化各影响因素的贡献。其他改进方向包括：构建物理启发混合模型、推广至其他城市群验证可迁移性、开发高分辨率预报接口等。