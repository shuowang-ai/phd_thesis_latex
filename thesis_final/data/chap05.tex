...
排放清单数据（$\mathbf{E}$）：排放驱动$\mathbf{E} \in \mathbb{R}^{N \times D_E}$（$D_E=5$）采用MEIC清单\citep{zheng2018trends}（详见第\ref{subsec:emis_data}节），包含PM$_{2.5}$、PM$_{10}$、SO$_2$、\NOx 和VOC等物种，原始月度数据通过时间降尺度转换为3小时分辨率。
...
在\ozone 变化方面，值得注意的是，北京和太原的\ozone 浓度呈现微弱的下降趋势，这与区域总体上升趋势形成对比。一种可能的解释是，北京和太原均处于VOC控制区（VOC-limited regime）\citep{li2019two}：在此类光化学环境中，\ozone 生成主要受VOC浓度制约，\NOx 减排反而会通过减弱\ozone 的化学滴定效应（NO + O$_3$ $\rightarrow$ NO$_2$ + O$_2$）导致\ozone 浓度上升。而随着减排策略持续推进，当\NOx 浓度降至足够低时，光化学体系逐步向\NOx 控制区过渡，\ozone 生成速率随\NOx 减少而降低，从而呈现出与其他VOC控制区城市不同的下降趋势。这一差异化响应表明，不同城市的最优减排路径取决于其所处的光化学敏感性区间，需要因地制宜地制定协同减排策略。
