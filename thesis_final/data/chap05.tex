...
谱图卷积：IGNN的空间建模采用第\ref{subsec:gnn_paradigms}节介绍的谱方法。与第\ref{chap:prediction}章和第\ref{chap:inference}章中使用的消息传递（MPNN）范式不同，本章选用切比雪夫谱卷积（公式\eqref{eq:chebynet}），其优势在于通过多项式阶数$K_c$精确控制空间感受野的跳数，使模型能够系统地聚合$K_c$阶邻域内的气象与排放信息。模型通过拉普拉斯矩阵将输入$(M, E)$变换到傅里叶空间\citep{kipf2017semi}：

\begin{equation}
\mathcal{G}_{\theta} * \mathcal{G}(M, E) = \mathbf{U}\mathcal{G}_{\theta}(\boldsymbol{\Lambda})\mathbf{U}^{T}(M, E)
\label{eq:ignn_spectral_conv}
\end{equation}

\noindent 式中$L = I_{N} - D^{-1/2}AD^{-1/2} = \mathbf{U}\boldsymbol{\Lambda} \mathbf{U}^{T}$为归一化图拉普拉斯矩阵。为降低计算复杂度，采用切比雪夫多项式近似\citep{kipf2017semi}：
