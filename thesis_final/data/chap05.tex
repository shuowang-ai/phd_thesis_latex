...
\begin{itemize}
    \item 气象驱动$\mathbf{M}^{\text{hist}}_{t-T'+1:t}$：来自ERA5再分析资料（2014-2020年），涵盖温度、风速、风向、降水、辐射、湿度、气压等7个变量；
    \item 排放驱动$\mathbf{E}^{\text{hist}}_{t-T'+1:t}$：来自MEIC清单（2014-2020年），涵盖\PM、PM$_{10}$、SO$_2$、\NOx、VOC等5个物种的排放强度。
\end{itemize}
...
\begin{itemize}
    \item 气象驱动$\mathbf{M}^{\text{future}}_{t-T'+1:t}$：来自CMIP6气候模式的RCP8.5高排放情景输出（CAS FGOALS-f3-L模型，2025-2050年）；
    \item 排放驱动$\mathbf{E}^{\text{future}}_{t-T'+1:t}$：来自DPEC模型的六种减排策略（从Baseline到1.5°C目标，2025-2050年）。
\end{itemize}
