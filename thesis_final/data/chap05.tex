...
针对上述挑战，本章提出IGNN模型，将排放清单作为可控变量显式纳入深度学习框架，通过\cqt{一对一映射}策略消除自回归误差累积，构建计算高效且精度可靠的代理模型。
...
如图\ref{fig:bthsa_network}所示，网络的构建与第\ref{chap:prediction}章（公式\eqref{eq:adjacency}）和第\ref{chap:inference}章（公式\eqref{eq:diffusion_adjacency}）采用相同的空间邻近性规则，距离阈值同为200 km。
...
气象再分析数据（$\mathbf{M}$）：气象驱动$\mathbf{M} \in \mathbb{R}^{N \times D_M}$（$D_M=7$）同样采用ERA5再分析数据集\citep{hersbach2020era5}（详见第\ref{subsec:meteo_data}节），选取气温、风速风向、降水、短波辐射、相对湿度和地表气压等变量，空间分辨率为$0.25^{\circ} \times 0.25^{\circ}$，时间分辨率为3小时。
...
模拟结果（如图\ref{fig:future_average}和表\ref{tab:future_trends_bthsa}所示）揭示了一个核心且值得高度关注的现象：\textbf{\PM 与\ozone 的浓度变化呈现出显著的相反趋势}。在所有六种排放策略下，\cqt{2+26}城市的年平均\PM 浓度均表现出显著的下降趋势，年均变化（AAC，Annual Average Change）在$-0.14$至$-0.37$~\ug之间（$p < 0.05$）。这表明，即使在全球持续高碳排放的背景下，只要实施既定的或更强化的污染物控制政策，\PM 治理仍能取得积极成效。

然而，与\PM 的下降趋势相反，年平均\ozone 浓度在除基准情景外的所有策略下都呈现出显著的上升趋势，AAC在$+0.07$至$+0.22$~\ug之间（$p < 0.05$）。这一结果预示着，未来\ozone 污染可能会取代\PM，成为该区域面临的主要大气环境挑战。
...
\begin{table}[htbp]
    ...
    \caption*{斜率单位为\ug{} (10 year)$^{-1}$，AAC（年均变化）单位为\ug{} year$^{-1}$，截距单位为\ug。斜率在$p < 0.05$水平显著的已加粗。六种策略按减排力度从弱到强排列。}
...
\begin{table}[htbp]
    ...
    \caption*{斜率为线性回归系数，AAC（年均变化）为首末差值除以年数，两者单位均为\ug{} year$^{-1}$，截距单位为\ug。斜率在$p < 0.05$水平显著的已加粗。}
...
结果与平均浓度的变化趋势高度一致。在所有策略下，\PM 污染日的年发生天数均呈现出显著的下降趋势。在三种最严格的减排策略下（2°C目标、碳中和目标和1.5°C目标），\PM 污染事件预计将在2030年前后被基本消除。
...
（3）\textbf{量化了不同城市的差异化响应。}不同城市间\ozone 变化幅度存在差异，体现了排放结构与光化学机制的区域异质性，为因地制宜制定减排策略提供了科学依据。在三种最严格的减排策略下（2°C目标、碳中和目标和1.5°C目标），\PM 污染事件预计将在2030年前后被基本消除。
