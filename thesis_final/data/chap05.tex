...
谱图卷积：IGNN的空间建模采用第\ref{subsec:gnn_paradigms}节介绍的谱方法。与第\ref{chap:prediction}章和第\ref{chap:inference}章中使用的消息传递（MPNN）范式不同，本章选用切比雪夫谱卷积（公式\eqref{eq:chebynet}），其优势在于通过多项式阶数$K_c$精确控制空间感受野的跳数，使模型能够系统地聚合$K_c$阶邻域内的气象与排放信息。模型通过拉普拉斯矩阵将输入$(\mathbf{M}, \mathbf{E})$变换到傅里叶空间\citep{kipf2017semi}：
...
\noindent 式中$\mathbf{L} = \mathbf{I}_{N} - \mathbf{D}^{-1/2}\mathbf{A}\mathbf{D}^{-1/2} = \mathbf{U}\boldsymbol{\Lambda} \mathbf{U}^{T}$为归一化图拉普拉斯矩阵。为降低计算复杂度，采用切比雪夫多项式近似\citep{kipf2017semi}：
...
\begin{table}[htbp]
    ...
    \caption*{斜率和年均变化（AAC）单位为\ug{} year$^{-1}$，截距单位为\ug。斜率在$p < 0.05$水平显著的已加粗。}
...
除了平均浓度的变化，污染事件的发生频率是衡量空气质量的另一重要维度。基于历史数据（2014-2020年），定义日最大污染物浓度超过第75百分位数的日期为\cqt{重污染日}，阈值分别为\PM{} $>$ 88~\ug 和\ozone{} $>$ 92~\ug。图\ref{fig:future_heavy_days}和表\ref{tab:future_heavy_days}展示了未来重污染日频率的模拟结果。

\begin{figure}[htbp]
  \centering
  \includegraphics[width=\linewidth]{figures/chap05_future_heavy_days.pdf}
  \caption{不同排放策略下未来重污染天数变化模拟}
...
\begin{table}[htbp]
    ...
    \begin{tabular}{@{}lc|c@{}}
        \toprule
        \textbf{排放策略} & \textbf{\PM{}重污染日 AAC} & \textbf{\ozone{}重污染日 AAC} \\
        \midrule
...
结果与平均浓度的变化趋势高度一致。在所有策略下，\PM 重污染日的年发生天数均呈现出显著的下降趋势。在三种最严格的减排策略下（1.5°C目标、2°C目标和碳中和目标），\PM 重污染事件预计将在2030年前后被基本消除。
...
（3）\textbf{量化了不同城市的差异化响应。}不同城市间\ozone 变化幅度存在差异，体现了排放结构与光化学机制的区域异质性，为因地制宜制定减排策略提供了科学依据。在三种最严格的减排策略下，\PM 重污染事件预计将在2030年前后被基本消除。
