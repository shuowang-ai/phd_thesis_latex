...
本文根据不同任务的特点灵活选择两种范式：第\ref{chap:prediction}章\ModelPred采用消息传递范式，利用其对边特征的灵活建模能力将风场编码为有向边权重；第\ref{chap:prediction}章PCDCNet采用基于拉普拉斯矩阵的一阶谱卷积，第\ref{chap:simulation}章IGNN采用ChebNet的多阶滤波特性捕捉多尺度空间依赖；第\ref{chap:inference}章SPIN则结合两种范式，分别建模扩散过程（谱卷积）和平流过程（有向消息传递）。
...
预测任务（第\ref{chap:prediction}章）：给定历史观测、未来气象和排放数据，预测未来污染物浓度。本文提出\ModelPred与PCDCNet模型，通过DIC损失显式约束时空连续性。
