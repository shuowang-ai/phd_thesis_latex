...
\begin{figure}[htbp]
    ...
    \caption{物理约束的时空图神经网络统一框架。蓝色模块为通用深度学习组件，橙色标注（$\star$）为本文的核心创新——在编码层、隐空间动力学、解码层三个层次融入物理约束。模型通过损失函数$\mathcal{L}$计算预测结果$\hat{\mathbf{X}}$与观测数据的差异，并通过反向传播更新参数$\Theta$，实现数据驱动的端到端优化。}
...
\textbf{（3）物理启发的领域知识}。阐述了对流-扩散方程的物理含义，建立了PDE各项与GNN模块的对应关系，提取了质量守恒这一核心物理约束，并从图结构设计、网络架构设计、损失函数设计和输入特征构建四个层面系统说明了物理知识融入图神经网络的具体途径。

\textbf{（4）本文研究范式}。提出了\cqt{物理约束的数据驱动建模}研究范式，在数据、模型、损失三个环节嵌入物理知识，构建了预测、推断、模拟三类任务的统一框架（图\ref{fig:unified_framework}），并与数值模式、纯数据驱动方法进行了对比分析。
