...
图神经网络的训练遵循深度学习的标准优化范式——基于梯度的反向传播算法\citep{goodfellow2016deep}。训练目标是最小化损失函数$\mathcal{L}(\Theta) = \frac{1}{M} \sum_{i=1}^{M} \ell(f_\Theta(\mathcal{G}_i), \mathbf{y}_i)$，其中$f_\Theta$为参数$\Theta$的GNN模型，$\ell(\cdot)$为任务相关的损失函数。反向传播算法通过链式法则高效计算梯度，并采用Adam等优化器\citep{kingma2015adam}更新参数$\Theta^{(t+1)} = \Theta^{(t)} - \eta \nabla_\Theta \mathcal{L}$。从系统科学角度，这种训练过程可理解为\textbf{从观测数据中反演系统动力学}——损失函数度量模型输出与真实系统状态的偏差，梯度信息指导参数调整，使模型逐步逼近真实系统的演化规律。
...
典型的物理约束项包括：\textbf{质量守恒约束}——基于连续方程$\frac{\partial C}{\partial t} + \nabla \cdot (C \mathbf{u}) = S$，约束空间传输满足\cqt{零和守恒}（本文第\ref{chap:prediction}章DIC损失）；\textbf{辅助观测约束}——利用遥感AOD等辅助数据源约束空间分布（本文第\ref{chap:inference}章）。
...
\begin{figure}[htbp]
    ...
    \caption*{蓝色模块为深度学习流水线（Pipeline），橙色标注（$\star$）为本文的核心创新——在编码层、隐空间动力学、解码层三个层次融入先验知识。模型通过损失函数$\mathcal{L}$计算预测结果$\hat{\mathbf{X}}$与观测数据的差异，并通过反向传播更新参数$\Theta$，实现数据驱动的端到端优化。}
