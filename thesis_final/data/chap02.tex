...
\label{eq:tcn}
\end{equation}
\noindent 式中，$\mathbf{Y}_t$为时刻$t$的输出，$\mathbf{X}_{t-d \cdot s}$为时刻$t-d \cdot s$的输入，$\mathbf{W}_s$为第$s$个卷积核权重，$\star$表示卷积操作，$d$为膨胀因子，$S$为卷积核大小。
...
典型的物理约束项包括：\textbf{质量守恒约束}——基于连续方程$\frac{\partial C}{\partial t} + \nabla \cdot (C \mathbf{u}) = Q$（$Q$为源汇项），约束空间传输满足\cqt{质量守恒}（本文第\ref{chap:prediction}章DIC损失）；\textbf{辅助观测约束}——利用遥感AOD等辅助数据源约束空间分布（本文第\ref{chap:inference}章）。
