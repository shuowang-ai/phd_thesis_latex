...
图神经网络的训练遵循深度学习的标准优化范式——基于梯度的反向传播算法\citep{goodfellow2016deep}。给定训练数据$\mathcal{D} = \{(\mathcal{G}_i, \mathbf{y}_i)\}_{i=1}^{M}$，其中$\mathcal{G}_i$为输入图，$\mathbf{y}_i$为目标标签，训练目标是最小化损失函数：
\begin{equation}
...
从系统科学角度，GNN的训练过程可理解为\textbf{从观测数据中反演系统动力学}。损失函数度量模型输出与真实系统状态的偏差，反向传播通过梯度信息指导参数调整，使模型逐步逼近真实系统的演化规律。这种\cqt{数据驱动的动力学学习}范式是本文方法论的核心思想。
...
大气污染物的时空演化遵循严格的物理化学规律，对流-扩散方程是描述这一过程的核心数学工具。本节阐述大气污染动力学的基本原理，为后续物理约束的设计提供理论依据。
...
\subsection{物理原理对深度学习建模的指导意义}
\label{subsec:physics_guidance}

上述大气污染动力学原理为本文的深度学习模型设计提供了明确的物理指导，具体体现在以下三个层面：

\textbf{（1）网络架构设计}。对流-扩散方程中各物理过程的算子结构可直接映射为神经网络模块：平流项的方向性特征启发了\textbf{风场驱动的有向图设计}（第\ref{chap:prediction}章PM$_{2.5}$-GNN的动态边权重、第\ref{chap:inference}章SPIN的平流核）；扩散项的各向同性特征对应\textbf{对称邻接矩阵的图卷积}（第\ref{chap:inference}章SPIN的扩散核）；化学反应的非线性特征则通过\textbf{多层感知机}建模（各章模型的节点级变换层）。

\textbf{（2）损失函数设计}。质量守恒约束可转化为训练目标中的\textbf{物理惩罚项}：第\ref{chap:prediction}章PCDCNet的DIC损失（Domain-Informed Constraints）约束预测结果的时空连续性；第\ref{chap:inference}章SPIN的AOD梯度损失利用遥感观测的空间结构信息引导推断结果符合物理分布规律。这些物理约束作为正则化项与监督损失共同优化，在保持数据拟合能力的同时增强模型的物理一致性。

\textbf{（3）输入特征构建}。排放源项$E$和气象驱动$\mathbf{u}$、$K$等物理量被显式编码为模型输入特征，使模型能够学习\cqt{驱动$\rightarrow$响应}的映射关系，而非仅仅拟合浓度时序的统计规律。这种设计使得模型具备对排放情景变化的响应能力，为第\ref{chap:simulation}章的假设情景模拟奠定基础。

综上，本节所阐述的大气污染动力学原理贯穿于后续各章的模型设计之中，构成了\cqt{物理约束数据驱动建模}范式的理论内核。
...
\subsection{核心思想：物理约束的数据驱动建模}
\label{subsec:core_idea}

本文提出的研究范式可概括为\textbf{物理约束的数据驱动建模}——在深度学习的黑盒优化框架中嵌入大气科学的领域知识，实现\cqt{数据拟合}与\cqt{物理一致}的双重目标。

纯数据驱动的深度学习模型虽然具有强大的拟合能力，但可能学习到违背物理规律的模式，尤其在数据稀疏或分布外样本上表现不佳\citep{karniadakis2021physics,kashinath2021physics}。物理约束融合旨在将领域知识嵌入深度学习框架，提升模型的物理一致性与泛化能力。

根据约束嵌入的方式与位置，本文在\textbf{数据、模型、损失}三个环节融入物理知识\citep{willard2022integrating}：

\textbf{数据环节}——在数据预处理或特征工程阶段融入物理知识。例如，基于风场信息构建有向图结构，将大气传输的方向性先验编码到数据表示中。

\textbf{模型环节}——在网络架构设计中嵌入物理结构。例如，将对流-扩散方程的算子结构映射到神经网络模块，使模型输出天然满足特定的物理对称性或守恒律。

\textbf{损失环节}——在训练目标中添加物理惩罚项。通过软约束方式鼓励模型输出满足物理规律，如质量守恒、时空连续性等。
