...
\textbf{排放源项$E$}刻画污染物的排放速率。现有数据驱动方法多采用\textbf{隐式代理}策略——通过时间特征或POI密度等间接变量近似排放的时空变化\citep{zheng2013u,yi2018deep,wang2021modeling}。这类方法本质上是学习\cqt{时间/空间$\rightarrow$浓度}的统计映射，\textbf{无法支撑未来情景模拟}。本文采用\textbf{显式排放数据}作为模型输入（第\ref{chap:prediction}--\ref{chap:simulation}章），使模型能够直接学习\cqt{排放$\rightarrow$浓度}响应映射，支撑假设情景模拟。
...
典型的物理约束项包括：\textbf{质量守恒约束}——基于连续方程$\frac{\partial C}{\partial t} + \nabla \cdot (C \mathbf{u}) = S$（$S$为源汇项），约束空间传输满足\cqt{质量守恒}（本文第\ref{chap:prediction}章DIC损失）；\textbf{辅助观测约束}——利用遥感AOD等辅助数据源约束空间分布（本文第\ref{chap:inference}章）。
