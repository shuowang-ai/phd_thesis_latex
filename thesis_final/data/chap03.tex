...
基于第\ref{chap:methodology}章的消息传递范式，构建空间图$\mathcal{G} = (\mathcal{V}, \mathcal{E})$。两节点$i$、$j$间建立边当且仅当：（1）地理距离$d_{ij} < 300$km（对应72小时内约4m/s风速的传输尺度）；（2）无高于1200m的山脉阻隔（物理屏障约束）。
...
阈值参数设置为$d_\theta = 300$km和$m_\theta = 1200$m。距离阈值对应污染物在典型风速（约4m/s）下72小时内的传输尺度；海拔阈值则反映山脉对边界层内污染物传输的阻隔效应。图\ref{fig:multi-image}展示了基于公式\eqref{eq:adjacency}构建的两个研究区域的城市网络结构。
