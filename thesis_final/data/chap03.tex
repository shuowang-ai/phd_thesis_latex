...
\end{equation}
\noindent 式中$\hat{\mathbf{X}}_i^{t-1}$为上一时刻的\PM 浓度预测值（或观测值），$\mathbf{M}_i^t$为当前时刻的气象特征向量。表\ref{tab:pm25gnn_node_attributes}列出了节点属性中包含的所有气象变量。

\begin{table}[htbp]
    \centering
    \caption{节点属性（$\mathbf{M}$）中包含的气象变量}
    \caption*{节点属性向量$\mathbf{M}_i^t \in \mathbb{R}^{8}$包含8个气象特征，用于刻画节点$i$在时刻$t$的局地气象条件及其对\PM 垂直扩散的影响。}
    \label{tab:pm25gnn_node_attributes}
    \begin{tabular}{@{}llp{5cm}@{}}
        \toprule
        \textbf{变量名称} & \textbf{单位} & \textbf{物理意义} \\
        \midrule
        边界层高度（BLH） & m & 决定污染物垂直混合的空间范围 \\
        K指数 & K & 反映大气层结稳定度 \\
        风速u分量 & m/s & 东西方向风速 \\
        风速v分量 & m/s & 南北方向风速 \\
        2m气温 & K & 影响化学反应速率和大气稳定度 \\
        相对湿度 & \% & 影响吸湿性增长和湿沉降 \\
        总降水量 & m & 湿沉降清除机制 \\
        地表气压 & Pa & 影响大气垂直运动 \\
        \bottomrule
