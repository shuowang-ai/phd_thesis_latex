...
\textbf{（4）实现细节。}模型基于PyTorch实现，图计算使用PyTorch Geometric（PyG）库\citep{fey2019fast}。训练在NVIDIA RTX 4070 SUPER GPU上进行。主要超参数设置：隐层维度$d=32$，历史窗口$T'=24$（72小时），预测窗口$T=24$（72小时），学习率$10^{-4}$，批大小32，训练轮数100，早停patience为10。优化器使用Adam，学习率调度使用ReduceLROnPlateau。
