...
本研究的核心贡献在于：提出物理启发的图神经网络建模范式，将图神经网络的关系建模能力与大气污染传输的网络化特征相结合，发展了预测、推断、模拟三位一体的空气质量智能建模方法体系。主要创新包括：（1）在图结构中显式编码\cqt{上风向影响下风向}的定向传输机制；（2）提出领域一致性约束（DIC）将物理守恒律嵌入模型训练；（3）提出以AOD空间梯度为约束的融合策略实现全天候推断；（4）首次将排放清单作为可控输入纳入深度学习框架，赋能假设情景模拟。完成从理论方法到业务系统的全链条落地，推动大气污染建模从\cqt{能预测}向\cqt{能推断、能模拟}跃升，为空气质量精细化管理提供智能技术支撑。本研究所发展的\cqt{物理先验驱动图结构与约束学习}方法论，可推广至地球科学乃至系统科学中涉及多源数据融合与复杂时空动力学建模的广泛问题，具有普适的理论与方法价值。
