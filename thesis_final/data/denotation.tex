...
\item[\(\mathbf{L}, \tilde{\mathbf{L}}\)] 图拉普拉斯矩阵与归一化拉普拉斯矩阵。非归一化形式$\mathbf{L} = \mathbf{D} - \mathbf{A}$；对称归一化形式$\mathbf{L} = \mathbf{I}_N - \mathbf{D}^{-1/2}\mathbf{A}\mathbf{D}^{-1/2}$（第\ref{chap:inference}、\ref{chap:simulation}章）。$\tilde{\mathbf{L}}$为切比雪夫多项式近似所用的缩放拉普拉斯矩阵$\tilde{\mathbf{L}} = \frac{2}{\lambda_{\max}}\mathbf{L} - \mathbf{I}_N$。
...
\item[\(\mathbf{H}^{(l)}, \mathbf{h}_i\)] 第$l$层隐表示，$\mathbf{h}_i$为节点$i$的隐向量。$\mathbf{h}_i \in \mathbb{R}^{d_h}$，其中$d_h$为隐层维度。
