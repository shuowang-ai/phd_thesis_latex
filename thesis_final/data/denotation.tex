...
\item[\(\mathcal{G}^{\text{s}}, \mathcal{G}^{\text{g}}\)] 站点图与网格图（第\ref{chap:inference}章）。$\mathcal{G}^{\text{s}}$以$N^{\text{s}}$个监测站点为节点，$\mathcal{G}^{\text{g}}$以$N^{\text{g}}$个$0.25^{\circ}$网格像素为节点，两者共享相同的图核构建方法与神经网络参数，但所得邻接矩阵维度不同。
