...
% 1. 核心变量
\item[\(\mathbf{X}_t, \hat{\mathbf{X}}_t\)] 时间 \(t\) 的空气质量观测值与预测值（单位：\ug{}），$\mathbf{X} \in \mathbb{R}^{N \times D_X}$，包含 PM\(_{2.5}\)、PM\(_{10}\)、O\(_3\)、NO\(_2\) 等污染物。
\item[\(X_v, \hat{X}_v\)] 节点 $v$ 的污染物浓度观测值与推断值，标量形式。$X_v \in \mathbb{R}$ 为节点级浓度观测，$\hat{X}_v \in \mathbb{R}$ 为模型输出的推断值（第\ref{chap:inference}章）。
...
\item[\(\mathbf{X}^{\mathrm{AOD}}\)] 遥感观测的气溶胶光学厚度（Aerosol Optical Depth），作为空间约束。附带节点级有效性掩码 $\boldsymbol{\omega}^{\mathrm{AOD}} \in \{0,1\}^{N}$（$\omega_i^{\mathrm{AOD}} = 1$表示有效，$0$表示云遮挡缺失），以及由此导出的边级掩码$\Omega_{ij}^{\mathrm{AOD}} = \omega_i^{\mathrm{AOD}} \cdot \omega_j^{\mathrm{AOD}}$\penalty10000{}。
...
\item[\(\mathbf{A}^{\text{s}}, \mathbf{A}^{\text{g}}\)] 站点图与网格图的基础邻接矩阵（第\ref{chap:inference}章），基于节点间测地距离$d_{ij}$按高斯核构建，$\mathbf{A}^{\text{s}} \in \mathbb{R}^{N^{\text{s}} \times N^{\text{s}}}$，$\mathbf{A}^{\text{g}} \in \mathbb{R}^{N^{\text{g}} \times N^{\text{g}}}$。
\item[\(\tilde{\mathbf{A}}^{\mathcal{D}}, \tilde{\mathbf{A}}^{\mathcal{A}}\)] 归一化扩散邻接矩阵与归一化平流邻接矩阵（第\ref{chap:inference}章）。$\tilde{\mathbf{A}}^{\mathcal{D}} = \mathbf{D}^{-1/2}\mathbf{A}\mathbf{D}^{-1/2}$建模各向同性扩散传播，$\tilde{\mathbf{A}}^{\mathcal{A}} = \mathbf{D}_{\text{in}}^{-1}\mathbf{A}^{\mathcal{A}}$建模风场驱动的各向异性平流传输。
\item[\(\mathbf{L}, \tilde{\mathbf{L}}\)] 图拉普拉斯矩阵与缩放拉普拉斯矩阵。非归一化形式$\mathbf{L} = \mathbf{D} - \mathbf{A}$；在第\ref{chap:inference}、\ref{chap:simulation}章中采用对称归一化形式$\mathbf{L} = \mathbf{I}_N - \mathbf{D}^{-1/2}\mathbf{A}\mathbf{D}^{-1/2}$。$\tilde{\mathbf{L}}$为切比雪夫多项式近似所用的缩放拉普拉斯矩阵$\tilde{\mathbf{L}} = \frac{2}{\lambda_{\max}}\mathbf{L} - \mathbf{I}_N$，其中$\lambda_{\max}$为$\mathbf{L}$的最大特征值。
\item[\(K\)] 大气扩散系数，用于对流--扩散方程的扩散项。
\item[\(t=0\)] 起报时刻，\(t \leq 0\) 为历史时刻，\(t>0\) 为未来预测时刻。
\item[\(T', T\)] 历史输入窗口与未来预测窗口长度。
\item[\(d_{ij}\)] 节点$i$与$j$之间的测地距离（球面大圆距离），用于图邻接矩阵的构建。
\item[\(D_X, D_M, D_E\)] 污染物、气象变量与排放变量的特征维度。
...
\item[\(\mathbf{H}^{(l)}, \mathbf{h}_i\)] 第$l$层隐表示，$\mathbf{h}_i$为节点$i$的隐向量。$\mathbf{h}_i \in \mathbb{R}^{d_h}$。
\item[\(d_h\)] 隐层维度，即各隐藏层特征向量的维数。
\item[\(R\)] 对流--扩散方程中的化学反应项。
\item[\(\mathcal{V}_{\text{obs}}, \mathcal{V}_{\text{target}}\)] 观测节点集与目标推断节点集。
\item[\(\nabla\)] 空间梯度算子，用于扩散/平流约束。
\item[\(\mathcal{L}\)] 总损失函数。各章节的具体构成不同：可能仅包含监督损失，也可能包含监督损失与物理约束损失的加权组合。
...
\item[\(\lambda, \lambda_1, \lambda_2\)] 损失函数中的平衡系数，控制数据拟合项与物理约束项之间的权重。
\item[\(r_{\text{mask}}\)] 动态节点掩码比例（第\ref{chap:inference}章），控制训练时随机遮蔽的站点比例。
\item[\(\ell_1, \ell_2\)] $\ell_1$损失（平均绝对误差MAE）与$\ell_2$损失（均方误差MSE）。本文采用$\ell_1$损失作为主要优化目标，以降低对异常值的敏感性。
...
\item[\ModelPred] \ensuremath{\mathrm{PM}_{2.5}}-GNN，本文提出的基于风场驱动图网络的预测前期模型（强基线）。
...
\item[IGNNK] Inductive Graph Neural Network Kriging，归纳式图核网络（Wu et al. 2021，第\ref{chap:inference}章基线方法，区别于本文IGNN）。
