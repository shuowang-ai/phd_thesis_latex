...
\begin{figure}[htbp]
  ...
  \caption*{图为彩云天气APP在生产环境中的界面截图。左图为污染峰值期间的截图，逐小时预报曲线显示AQI处于高位；右图为同一污染过程消散后的截图，曲线已回落至优良区间。底部地图中颜色表示\PM 浓度，红色/紫色表示高浓度，绿色表示低浓度。两图中预报曲线的走势与对应时刻的实测值高度吻合，且所涉数据均未出现在模型训练集中，验证了模型的泛化能力与预报稳定性。}
  \label{fig:beijing_pollution_case}
\end{figure}

图\ref{fig:beijing_pollution_case}给出了一次典型重污染事件的预报案例。左图为污染峰值期间的APP截图，小时预报曲线显示AQI正处于高位并将在未来数小时内维持较高水平；右图为同一污染过程消散后的截图，曲线已回落至优良区间。对比两图中的逐小时预报曲线与对应时刻的实测AQI值可以发现，系统在污染发生前数日即准确预判了污染物浓度的上升—峰值—下降这一完整演变过程，预报曲线的走势与实际观测高度吻合。值得强调的是，该预报结果来自已上线运行的生产环境，图中所涉及的时段和区域数据均未出现在模型的训练数据集中，属于模型在真实业务场景下对全新数据的推断。这一结果充分验证了模型在实际部署条件下的泛化能力与预报稳定性，表明其具备良好的实用价值。
...
（2）在多个高标准实战场景中验证了模型性能。KnowAir系统（核心为本文第\ref{chap:prediction}章提出的PCDCNet模型）在多个国家级应用场景中经受了严格检验。在上海进博会保障的定量评估中，KnowAir相较于\CMAQ、WRF-Chem、NAQPMS等业务化数值模式，在\PM、\ozone、\PMten{}等关键污染物的预报上展现出显著优势（\PM 的RMSE从14.5--25.1降低至5.7，相关系数从0.395--0.641提升至0.820）；在粤港澳官方模型比对测试中，KnowAir取得综合评分中位数第一名。
