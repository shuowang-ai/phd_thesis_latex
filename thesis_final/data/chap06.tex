...
为评估模型的实战表现，建立了覆盖全国主要城市的在线预报性能监测体系。图\ref{fig:online-aqi-metrics}展示了北京、上海、石家庄、郑州四个代表性城市的AQI预报性能，短期预报与实测值高度吻合，中长期预报仍能较好把握污染变化趋势。
...
\begin{figure}[htbp]
  ...
  \subcaptionbox{上海市\label{fig:online-aqi-shanghai}}
    {\includegraphics[width=0.48\linewidth]{figures/chap06_aqi_shanghai.png}}
  \caption{代表性城市AQI预报性能时序对比}
  \caption*{横轴为时间，纵轴为AQI值。各子图中黑色曲线为实际观测值（OBS），彩色曲线分别为提前3小时、6小时、12小时、24小时、48小时、72小时发布的预报值。例如，\cqt{3h}曲线表示在实际时刻前3小时发布的预报结果。模型在短期预报（3--12h）中几乎与观测重合，随预报时效延长（24--72h）仍保持良好的趋势把握能力。}
  \label{fig:online-aqi-metrics}
\end{figure}

值得注意的是，臭氧对数据驱动模型与传统数值模型呈现截然相反的预报难度特性。对于AI模型，臭氧的可预报性优于\PM，因为臭氧浓度具有高度规律的日周期变化模式，深度学习模型擅长捕捉这种周期性规律。而对于CMAQ等传统数值模型，臭氧预报是公认的技术难点——需耦合包含上百个化学方程的气相机理，且高度依赖前驱物排放清单的准确性。数据驱动方法通过端到端学习规避了显式求解光化学方程组的需求，在保持精度的同时降低了对排放清单的敏感性。
