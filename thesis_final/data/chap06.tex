...
\begin{figure}[htbp]
  ...
  \caption*{各子图展示了不同预报时效（3h--72h）下的预测值与实测值对比。模型在短期预报中表现出色，随预报时效延长仍保持良好的趋势把握能力。}
...
\caption*{在长三角地区测试中，KnowAir模型在三种污染物预报任务上均取得最优性能。RMSE为均方根误差（$\downarrow$越小越好），R为相关系数（$\uparrow$越大越好）。}
...
在\cqt{彩云天气}APP中，KnowAir模型为数千万用户提供空气质量预报服务，主要功能包括：基于SPIN模型的高分辨率污染物空间分布图、未来多日逐小时污染物浓度预报，以及重污染天气的提前预警。
