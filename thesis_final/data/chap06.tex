...
本章阐述了将深度学习模型从理论研究推向真实世界应用的全过程。

（1）设计并实现了云原生的低成本自动化部署系统。微服务架构将数据接入、模型训练、在线推理和API服务解耦为独立组件；Kubernetes容器编排提供弹性扩展能力；GitOps流程打通了代码提交到生产部署的自动化链路。相较传统CMAQ{+}WRF系统需专业团队驻场运维，本系统仅需普通配置云服务器即可运行，运维成本降低90\%以上。
...
上述工作表明，物理启发的深度学习方法经过工程化部署后，能够在实际业务环境中保持与离线实验一致的精度与稳定性。