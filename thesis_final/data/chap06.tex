...
在前期针对长三角地区的严格测试中，本AI模型相较于多款业务化运行的数值模型（\CMAQ、WRF-Chem、NAQPMS），在\PM、\ozone、\PMten等关键污染物的预报上均取得了更优的精度。
...
图\ref{fig:caiyun_web}为面向公众的空气质量可视化平台界面\footnote{\url{https://caiyunapp.com/map/}}。该平台基于KnowAir系统的输出，提供全国范围的实时空气质量分布图，支持\PM、\ozone、\PMten等多污染物切换及未来多日的时间动画播放功能。
...
（2）在多个高标准实战场景中验证了模型性能。KnowAir系统（核心为本文第\ref{chap:prediction}章提出的PCDCNet模型）在多个国家级应用场景中经受了严格检验。在上海进博会保障的定量评估中，KnowAir相较于\CMAQ、WRF-Chem、NAQPMS等业务化数值模式，在\PM、\ozone、\PMten等关键污染物的预报上展现出显著优势（\PM 的RMSE从14.5--25.1降低至5.7，相关系数从0.395--0.641提升至0.820）；在粤港澳官方模型比对测试中，KnowAir取得综合评分中位数第一名。
