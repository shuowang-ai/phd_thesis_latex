...
\caption*{\textbf{SPIN}：算法包含四个核心阶段：动态节点掩码（模拟数据缺失场景）、TCN时序编码（提取本地物理特征）、双图传播（物理引导的空间信息交互）、以及复合损失优化（融合推断损失、初始化损失和AOD梯度约束）。}
\label{alg:spin}
\begin{algorithmic}[1]
    ...
    \FOR{每个样本$D_k = (\mathbf{X}, \mathbf{M}, \mathbf{E}, \mathbf{X}^{\text{AOD}}) \in \mathcal{D}$}
        ...
        \FOR{$l \leftarrow 1$ to $L$}
            \STATE $\mathbf{H}^{\mathcal{D}} \leftarrow \tilde{\mathbf{A}}^{\mathcal{D}} \mathbf{W}^{\mathcal{D}} \mathbf{H}^{(l-1)}$ \COMMENT{扩散消息}
            \STATE $\mathbf{H}^{\mathcal{A}} \leftarrow \tilde{\mathbf{A}}^{\mathcal{A}} \mathbf{W}^{\mathcal{A}} \mathbf{H}^{(l-1)}$ \COMMENT{平流消息}
            \STATE $\mathbf{G} \leftarrow \sigma(\mathbf{W}_g [\mathbf{H}^{\mathcal{D}}, \mathbf{H}^{\mathcal{A}}])$ \COMMENT{门控权重}
            \STATE $\Delta\mathbf{H} \leftarrow \mathbf{G} \odot \mathbf{H}^{\mathcal{D}} + (1-\mathbf{G}) \odot \mathbf{H}^{\mathcal{A}}$ \COMMENT{融合}
            \STATE $\mathbf{H}^{(l)} \leftarrow \text{ReLU}(\mathbf{H}^{(l-1)} + \Delta\mathbf{H})$ \COMMENT{残差更新}
        \ENDFOR
