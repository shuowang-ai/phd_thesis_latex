...
本阶段同样服务于站点推断和网格化推断，两者共享相同的扩散核与平流核。经过阶段二的编码后，观测节点携带真实的污染信息，而目标节点（站点或网格像素）仅携带环境背景信息。本阶段通过物理启发的图传播机制，将观测节点的污染信息沿物理合理的路径传递至目标节点。
...
经过$L$层物理启发的空间传播后，由于目标节点本身没有任何历史观测数据，无法采用自回归方式。因此设计了一个节点共享的读出层（Readout Layer），由多层感知机构成，直接将高维潜在特征映射为污染物浓度：
\begin{equation}
...
\begin{figure}[htbp]
    ...
    \caption*{下图（张家口人民公园站）展示了在极度缺乏上游信息时，物理启发机制确保了模型输出合理的背景趋势，而非随机噪声或幻觉值。}
