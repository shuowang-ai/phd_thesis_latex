...
（1）时变物理机理的方向性建模。污染物受风场驱动的平流输送具有强烈的各向异性，而传统基于欧氏距离的插值方法（如反距离加权（Inverse Distance Weighting, IDW）、克里金（Kriging））假设空间平稳性，难以捕捉\cqt{上风向影响下风向}的定向传输特性。如何将公式（\ref{eq:advection_diffusion}）中的平流项与扩散项显式解耦，分别设计各向异性的平流传播核与各向同性的扩散传播核，是物理约束嵌入的核心问题。

（2）遥感数据的非随机缺失与融合。卫星AOD虽能提供面状观测，但在云层遮挡、夜间等条件下大面积缺失，缺失模式呈现系统性的时空偏差\citep{van2016global,xiao2017full}。此外，AOD作为柱积分量与近地面浓度之间存在复杂的非线性关系，受气溶胶类型、垂直分布和大气湿度等因素调制。如何将AOD转化为空间结构约束而非强制输入，在保留其空间引导价值的同时规避缺失数据对模型的不利影响，是多源数据融合的核心问题。

（3）归纳式空间泛化。尽管中国已建成全球规模最大的空气质量监测网络（约1618个国控站点），但超过80\%的人口仍生活在缺乏直接监测的区域\citep{southerland2022global,wei2023first}。现有图神经网络多属于直推式（Transductive）学习框架，无法泛化至训练集中未出现的新节点\citep{wu2021inductive}。如何设计归纳式（Inductive）学习策略，使模型能够对任意未观测位置进行空间推断，是突破模型泛化瓶颈的核心问题。

针对上述挑战，现有方法存在以下不足：（1）在物理机理建模方面，数值化学传输模式（如CMAQ）虽具有完备的物理方程，但高分辨率全域模拟的计算成本限制了其在实时推断中的应用；传统空间插值方法（IDW、Kriging）基于空间平稳性假设，隐含各向同性前提，无法刻画风场驱动的定向传输特性\citep{li2011review}。（2）在空间传输建模方面，现有时空图神经网络多采用基于欧氏距离的静态图结构，尽管第\ref{chap:prediction}章已在预测任务中引入风驱动的动态图机制，但如何将平流-扩散双重物理过程显式解耦并嵌入归纳式推断框架，仍是尚未解决的难题。（3）在遥感数据利用方面，传统方法将AOD作为模型的强制输入特征，在云层遮挡或夜间等数据缺失时段完全失效\citep{just2020gradient}，难以实现全天候的空间推断。
...
\begin{figure}[htbp]
    ...
    \caption*{输入：观测站点历史时序$\mathbf{X}_{\text{obs}}^{t-T'+1:t}$（黑色圆点）、气象场$\mathbf{M}_F^{t-T'+1:t}$、排放场$\mathbf{E}_F^{t-T'+1:t}$、卫星AOD $\mathbf{X}^{\text{AOD},t-T'+1:t}$（斜线区域表示缺失）；输出：目标站点推断$\hat{\mathbf{X}}_{\text{target}}^{t}$（蓝色曲线）及网格推断$\hat{\mathbf{X}}_F^t$（红色热力图）；模型：基于扩散-平流双图机制构建图结构$\mathcal{G}$，通过$\mathcal{F}_\Theta$（SPIN）建模节点间的时空相关性。}
    \label{fig:inference_problem}
\end{figure}

如图所示，空间推断问题的输入-输出结构如下：输入包括四类数据——观测站点的历史时序数据$\mathbf{X}_{\text{obs}}^{t-T'+1:t}$（图中黑色圆点表示的观测节点）、全域气象场$\mathbf{M}_F^{t-T'+1:t}$（包括风速风向、温度、湿度、边界层高度等）、排放场$\mathbf{E}_F^{t-T'+1:t}$以及卫星遥感AOD数据$\mathbf{X}^{\text{AOD},t-T'+1:t}$（图中斜线区域表示缺失）；输出包括未观测站点的\PM 浓度时序推断（图中蓝色曲线）及全域网格化\PM 空间分布$\hat{\mathbf{X}}_F^t$（图中红色热力图）。

与第\ref{chap:prediction}章的\emph{时间维度预测}不同，本章聚焦于\emph{空间维度推断}。类似于第\ref{chap:prediction}章，模型同样采用历史时间窗口作为输入：给定任意时刻$t$之前$T'$个时间步（$t-T'+1, \ldots, t$）观测站点的历史数据，推断该时刻$t$未观测位置的浓度值。时间窗口的引入使模型能够捕捉污染物的时序演化特征，从而提升空间推断的准确性。这种设计使得模型能够同时服务于历史重建（用于暴露评估）和实时监测（用于填补监测盲区）两类应用场景。

该问题包含两个层次化的子任务：

（1）站点推断（Station Inference）。推断保留测试集（Held-out）中未观测站点的时序数据，用于评估模型的插值泛化能力。这一任务模拟的场景是：当新建一个监测站点时，如何基于周边已有站点的观测数据估算其污染水平。

（2）网格推断（Grid Inference）。对区域内所有$0.25^{\circ}$网格点进行推断，生成连续的高分辨率污染场$\hat{\mathbf{X}}_F^t$，以消除监测盲区。这一任务的应用场景包括：生成空间连续的污染暴露评估图、识别缺乏监测的污染热点、支撑精细化的区域联防联控决策。
...
传统的纯数据驱动模型往往将大气污染推断视为一个黑盒的时空回归问题，忽略了支配污染物传输的物理规律。针对这一局限，本节详细阐述物理引导的归纳式图推断网络（SPIN）的构建细节。该模型遵循第\ref{subsec:paradigm}节提出的\cqt{物理约束的数据驱动建模}范式，将大气流体动力学中的扩散（Diffusion）与平流（Advection）机制显式地映射为图神经网络中的传播算子。
...
（1）时序特征编码器（Temporal Feature Encoder）。负责从各节点本地的气象与排放时间序列中提取高维动力学特征，为后续的空间传播提供物理信息丰富的初始状态。该模块对应于第\ref{chap:methodology}章图\ref{fig:unified_framework}所示统一框架中的编码层（Encoder），将多源异构数据映射到统一的隐空间。

（2）物理引导的图传播模块（Physics-Guided Graph Propagation）。这是模型的核心，对应于统一框架中的隐空间动力学（Latent Dynamics）层。它通过并行设计的\cqt{扩散核}和\cqt{平流核}，在图结构上模拟污染物的各向同性扩散和各向异性平流输送过程，分别对应公式（\ref{eq:advection_diffusion}）中的扩散项与平流项。

（3）读出与生成模块（Readout \& Generation）。对应于统一框架中的解码层（Decoder），将经过时空交互后的潜在表征映射回污染物浓度空间，生成最终的推断结果。
...
这种架构设计使得SPIN与PCDCNet形成了互补关系：PCDCNet通过LID--STD--TAD三模块解耦时间维度的化学反应、空间传输和累积过程，用于预测已知站点的未来状态；而SPIN通过扩散--平流双核解耦空间维度的各向同性与各向异性传输过程，用于推断未知位置的当前状态。两者共享相同的物理先验——对流--扩散方程，但在技术实现上各有侧重。
...
（1）地理空间图（Geospatial Graph, $\mathcal{G}^{\mathcal{D}}$）。基于地理学第一定律\cqt{相近事物更相关}（Tobler's First Law of Geography），我们首先构建基础的地理空间图。邻接矩阵$A^{\mathcal{D}}$基于节点间的测地距离定义：
\begin{equation}
...
（2）扩散核（Diffusion Kernel, $\tilde{A}^{\mathcal{D}}$）。扩散过程是指污染物在浓度梯度驱动下产生的各向同性（Isotropic）随机运动，对应公式（\ref{eq:advection_diffusion}）中的扩散项$K_h\left(\frac{\partial^2 C}{\partial x^2} + \frac{\partial^2 C}{\partial y^2}\right)$。在图信号处理中，这一过程对应于图拉普拉斯算子的平滑作用\citep{kipf2017semi}。我们通过对$A^{\mathcal{D}}$进行对称归一化来构造扩散算子：
\begin{equation}
...
在GNN的消息传递中，$\tilde{A}^{\mathcal{D}}$使得节点能够聚合来自所有方向邻居的信息，模拟了大气湍流导致的污染物浓度均一化过程。这种对称归一化的设计确保了扩散过程是双向的、各向同性的——符合公式（\ref{eq:advection_diffusion}）中扩散项的物理特性。


\subsubsection{平流核与各向异性传输}

传统的GNN往往只考虑距离，隐含了各向同性的假设。然而，在大气环境中，风场主导的平流输送具有强烈的各向异性（Anisotropic）——上游节点对下游节点的影响远大于反向影响，对应公式（\ref{eq:advection_diffusion}）中的平流项$-\left(u\frac{\partial C}{\partial x} + v\frac{\partial C}{\partial y}\right)$。

为了建模这一关键物理过程，我们设计了平流图（Advection Graph, $\mathcal{G}^{\mathcal{A}}$）。这是一个动态的有向图，其边权重取决于风速向量在节点连线上的投影。如图~\ref{fig:kernels}(a)所示，从节点$j$到节点$i$的平流权重定义为：
\begin{equation}
...
\begin{figure}[htbp]
    ...
    \caption*{(a) 平流核（Advection Kernel）：通过计算风速向量在节点连线方向上的投影，构建有向图结构，显式建模污染物的定向输送。(b) AOD梯度损失：利用卫星AOD的空间梯度约束预测场，仅在有效观测区域（阴影部分）计算损失，以应对云层遮挡。}
    \label{fig:kernels}
\end{figure}
...
% 读出层与预测生成
经过$L$层物理引导的空间传播后，每个节点的最终潜在表示$\mathbf{H}_i^{(L)}$已经深度融合了：（1）TCN提取的本地气象与排放动态；（2）GNN从邻域聚合的外部污染信息。

由于在推断任务中，目标节点（Target Nodes）本身没有任何历史观测数据（即$\mathbf{X}_{\text{history}}$缺失），我们无法采用自回归的方式。因此，设计了一个节点共享的读出层（Readout Layer），由多层感知机（MLP）构成，直接将高维潜在特征映射为污染物浓度：
\begin{equation}
...
为了赋予模型对未观测节点的推断能力，采用了动态节点掩码（Dynamic Node Masking）策略\citep{wu2021inductive}。在每一轮训练迭代（Iteration）中，从所有已知观测数据的站点集合$\mathcal{V}_{\text{obs}}$中，随机采样一个子集（例如50\%）作为\cqt{掩码目标节点（Masked Target Nodes）}，记为$\mathcal{V}_{\text{mask}}$。

归纳式训练的核心思想是：在每个训练批次中，随机将监测站点划分为观测节点和目标节点：
\begin{equation}
...
（3）全天候适用。在夜间或云覆盖时，$\mathcal{L}_{\mathrm{AOD}}$约束自动失效（因为$\boldsymbol{\Omega}^{\text{AOD}}$全为零），模型平滑回退到由物理核（扩散--平流）驱动的推断模式，仅凭气象场和地面站点生成了物理上合理的平滑场。这种\cqt{优雅降级}（Graceful Degradation）特性是本方法相比传统AOD输入方法的核心优势。
...
本章的最终目标是生成连续的网格化污染地图。图~\ref{fig:grid_inference}通过四个典型案例，深入剖析了SPIN如何利用物理核与AOD梯度损失来处理卫星数据的缺陷。需要强调的是，在本框架中，AOD不作为推理时的直接输入特征，而是作为训练时的空间梯度约束。因此，图中的AOD面板作为外部参考，用于验证模型是否成功内化了污染的空间结构模式。
...
（1）构建了物理嵌入的双流图网络架构。通过显式设计扩散核与平流核，模型成功解耦了浓度梯度驱动的各向同性扩散（公式\eqref{eq:diffusion_normalized}）与风场驱动的各向异性平流输送（公式\eqref{eq:advection_adjacency}）两类物理过程，分别对应公式（\ref{eq:advection_diffusion}）中的扩散项与平流项。平流核根据实时风场动态调整边权重，显式编码\cqt{上游影响下游}的定向传输规律；扩散核模拟浓度梯度驱动的均一化过程，在数据稀疏区域起到\cqt{保守滤波}作用。这种设计与第\ref{chap:prediction}章PM$_{2.5}$-GNN和PCDCNet的物理约束思想一脉相承。

（2）实现了归纳式时空推断能力。通过动态掩码训练策略（公式\eqref{eq:inductive_sampling}），模型摆脱了对固定图结构的依赖，具备了对任意空间位置进行\cqt{虚拟传感}的能力。实验证明，该模型在未观测站点上的推断精度显著优于现有最优方法（MAE降低25.2\%），特别是在冬季复杂气象条件下表现出极强的鲁棒性。

（3）提出了全天候制图的AOD融合范式。创新性地提出\cqt{掩码AOD梯度损失}（公式\eqref{eq:aod_loss}），实现了从\cqt{依赖卫星输入}到\cqt{利用卫星约束}的范式转换。这一机制不仅有效利用了晴空下的卫星空间信息，更在卫星数据缺失（云/夜）时，保证了系统能够平滑回退到物理驱动模式，从而生成了时空连续、物理一致的高分辨率污染地图。
