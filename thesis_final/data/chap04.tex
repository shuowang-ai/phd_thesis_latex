...
\textbf{阶段一：动态节点掩码}（解决问题一）。在每个训练批次中，随机将部分监测站点标记为\cqt{目标节点}（模拟无观测状态），迫使模型学习在缺乏直接观测时的推断能力，实现归纳式空间泛化。

\textbf{阶段二：本地物理特征编码}（服务两个问题）。利用时间卷积网络从各节点的气象和排放时间序列中提取物理特征，为包括无观测节点在内的所有位置提供基于本地环境条件的初始浓度估计。

\textbf{阶段三：物理引导的双图空间传播}（解决问题一）。设计扩散核与平流核分别建模各向同性扩散和各向异性平流过程，显式解耦公式（\ref{eq:advection_diffusion}）中的扩散项与平流项，通过图传播将观测节点的信息传递至无观测区域。

\textbf{阶段四：多层级约束与AOD融合}（解决问题二）。设计包含推断损失、初始化损失和掩码AOD梯度损失的复合约束体系。其中AOD梯度损失创新性地将卫星数据作为空间结构约束而非强制输入，在AOD可用时提取其空间分布信息，在AOD缺失时自动回退至物理驱动模式，实现全天候推断。
...
\textbf{解决的问题：}问题一（稀疏监测网络下的空间泛化推断）。为了使模型具备对未观测节点的推断能力，采用了动态节点掩码（Dynamic Node Masking）策略\citep{wu2021inductive}。与传统的直推式（Transductive）方法不同，本策略不依赖固定的图结构，而是通过模拟数据缺失场景，强迫模型学习通用的时空插值规律。
...
\textbf{解决的问题：}服务于两个核心问题——为所有节点（包括阶段一中被掩码的目标节点）提供基于本地物理条件的初始浓度估计。
...
\textbf{解决的问题：}问题一（稀疏监测网络下的空间泛化推断）。经过阶段二的编码后，观测节点携带真实的污染信息，而目标节点仅携带环境背景信息。本阶段通过物理引导的图传播机制，将观测节点的污染信息沿物理合理的路径传递至目标节点。
...
\textbf{解决的问题：}问题二（非随机缺失遥感数据的有效融合）。经过阶段三的空间传播后，每个节点的潜在表示$\mathbf{H}_i^{(L)}$已深度融合了本地物理特征和邻域污染信息。本阶段首先通过读出层将潜在表征映射为浓度值，然后通过精心设计的多层级约束体系实现AOD数据的有效融合。
