...
\end{equation}
\noindent 式中$D_{\text{out},ii} = \sum_j A_{ij}^{\mathcal{A}}$。行归一化确保每个节点的出流总和为1，符合质量守恒的物理约束——这与第\ref{chap:prediction}章第\ref{subsec:pcdcnet_training}节讨论的DIC约束在理念上一致。
...
\label{eq:aod_mask}
\end{equation}
\noindent 当且仅当边$(i,j)$两端节点均有有效AOD数据时$\Omega_{ij}^{\text{AOD}} = 1$，则AOD空间梯度损失项为：
\begin{equation}
...
\begin{figure}[htbp]
    ...
    \caption*{左图为模型预测浓度场在网格图上沿边$(i,j)$的空间梯度$\nabla_{ij}(\hat{\mathbf{X}}) = \hat{X}_j - \hat{X}_i$；右图为对应的卫星AOD场梯度$\nabla_{ij}(\mathbf{X}^{\text{AOD}})$，其中斜线区域表示AOD数据可用（$\boldsymbol{\omega}^{\text{AOD}} = 1$）的网格节点。损失函数$\mathcal{L}_{\mathrm{AOD}}$（公式\eqref{eq:aod_loss}）仅在两端点均有有效AOD观测（$\Omega^{\text{AOD}}_{ij}=1$）的边上计算，从而避免对缺失区域的错误约束。}
