...
\begin{align}
    \text{站点推断：} \quad \hat{\mathbf{X}}^{\text{target}}_{t-T'+1:t} &= \mathcal{F}^{\text{s}}_{\Theta^{\text{s}}}(\mathbf{X}^{\text{obs}}_{t-T'+1:t}, \mathbf{M}^{F}_{t-T'+1:t}, \mathbf{E}^{F}_{t-T'+1:t}, \mathcal{G}^{\text{s}}) \label{eq:infer_problem} \\
...
\end{equation}
\noindent 式中$d_{ij}$为节点$i$和$j$之间的测地距离，$\sigma_d$为距离衰减系数（默认50 km），$d_{\text{th}}$为截断阈值（默认200 km）。在此基础图结构上，第\ref{sec:spin_model}节的阶段三将进一步构建两类物理传播核——扩散核与平流核（第\ref{subsec:stage3_propagation}节）。
