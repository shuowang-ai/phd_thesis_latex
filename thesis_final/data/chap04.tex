...
需要强调的是，虽然两个子任务使用不同粒度的图$\mathcal{G}^{\text{s}}$和$\mathcal{G}^{\text{g}}$，但它们采用\textbf{完全相同的模型架构}和\textbf{图核构建方法}（扩散核公式\eqref{eq:diffusion_adjacency}--\eqref{eq:diffusion_normalized}与平流核公式\eqref{eq:advection_adjacency}--\eqref{eq:advection_normalized}），仅在输入节点集合、训练数据与损失函数上有所区别（网格推断额外引入AOD空间约束损失），且各自独立训练。由于节点集合不同，两套图上所得的邻接矩阵维度不同。其中$\tilde{A}^{\mathcal{D}}$为归一化扩散邻接矩阵，刻画污染物在浓度梯度驱动下的各向同性扩散传播；$\tilde{A}^{\mathcal{A}}$为归一化平流邻接矩阵，刻画风场驱动下的定向平流传输（两者的详细构建方法见第\ref{subsec:stage3_propagation}节）。在$\mathcal{G}^{\text{s}}$上$\tilde{A}^{\mathcal{D}}, \tilde{A}^{\mathcal{A}} \in \mathbb{R}^{N^{\text{s}} \times N^{\text{s}}}$，在$\mathcal{G}^{\text{g}}$上$\tilde{A}^{\mathcal{D}}, \tilde{A}^{\mathcal{A}} \in \mathbb{R}^{N^{\text{g}} \times N^{\text{g}}}$。这种设计体现了SPIN的核心思想：同一物理传输架构可在不同空间尺度上复用。
...
排放数据（$\mathbf{E} \in \mathbb{R}^{N \times D_E}$）采用与第\ref{chap:prediction}章相同的MEIC清单\citep{zheng2018trends,li2017anthropogenic}，包含\NOx{}、\VOC{}、\SOtwo{}、\NHthree{}及一次\PM 等主要污染物的网格化排放，并通过时间分配方法降尺度为小时分辨率（详见第\ref{subsec:emis_data}节）。排放数据通过提供污染物的\cqt{源强}信息，帮助模型学习\cqt{高排放区域污染水平通常较高}的关联规律，对于推断缺乏监测的工业区域尤为重要。
...
在进入推断流水线之前，需要为两个子任务分别构建空间传播的基础图结构。如图~\ref{fig:study_area}所示，两套图虽然节点集合不同——站点图$\mathcal{G}^{\text{s}}$以监测站点为节点（图~\ref{fig:study_area}(b)），网格图$\mathcal{G}^{\text{g}}$以$0.25^{\circ}$网格像素为节点（图~\ref{fig:study_area}(d)）——但\textbf{采用完全相同的连边规则和邻接矩阵构建方法}，即下文公式\eqref{eq:diffusion_adjacency}。区别仅在于：节点集合不同导致边集合的具体构成和邻接矩阵的维度不同（$A^{\text{s}} \in \mathbb{R}^{N^{\text{s}} \times N^{\text{s}}}$ vs.\ $A^{\text{g}} \in \mathbb{R}^{N^{\text{g}} \times N^{\text{g}}}$）。
...
两套图的邻接矩阵$A$（$A \in \{A^{\text{s}}, A^{\text{g}}\}$）均基于地理学第一定律\cqt{相近事物更相关}（Tobler's First Law of Geography），按节点间的测地距离以相同的方式定义：
\begin{equation}
    A_{ij} = \begin{cases}
...
扩散核建模污染物在浓度梯度驱动下产生的各向同性随机运动。基于第\ref{subsec:spin_graph}节的距离邻接矩阵$A$（公式\eqref{eq:diffusion_adjacency}），对其进行对称归一化，构造扩散算子$\tilde{A}^{\mathcal{D}}$：
\begin{equation}
    \tilde{A}^{\mathcal{D}} = \mathbf{D}^{-\frac{1}{2}} A \mathbf{D}^{-\frac{1}{2}}
\label{eq:diffusion_normalized}
\end{equation}
\noindent 式中$\mathbf{D}$是$A$的度矩阵。该对称归一化对应于图拉普拉斯算子的平滑作用\citep{kipf2017semi}，使得节点能够聚合来自所有方向邻居的信息，模拟了公式\eqref{eq:advection_diffusion}中扩散项$K_h\left(\frac{\partial^2 C}{\partial x^2} + \frac{\partial^2 C}{\partial y^2}\right)$所描述的大气湍流导致的浓度均一化过程。扩散核为静态核，在模型整个生命周期内保持不变。
...
归一化采用行归一化（入度归一化）：
\begin{equation}
\tilde{A}^{\mathcal{A}} = \mathbf{D}_{\text{in}}^{-1} A^{\mathcal{A}}
