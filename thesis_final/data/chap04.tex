...
问题二：非随机缺失遥感数据的有效融合。卫星AOD虽能提供面状观测以弥补地面监测的空间盲区，但在云层遮挡、夜间等条件下大面积缺失，缺失模式呈现系统性的时空偏差\citep{van2016global,xiao2017full}。此外，AOD作为柱积分量与近地面浓度之间存在复杂的非线性关系，受气溶胶类型、垂直分布和大气湿度等因素调制。传统方法将AOD作为模型的强制输入特征，在数据缺失时段完全失效\citep{just2020gradient}，难以实现全天候的空间推断。本章面临的第二个核心问题是：如何将AOD转化为空间结构约束而非强制输入，在保留其空间引导价值的同时规避缺失数据对模型的不利影响？

上述两个问题相互制约又相互促进：问题一要求模型能够在无观测区域进行推断，而问题二中的AOD数据恰好能为无观测区域提供空间结构线索；然而AOD本身的缺失问题又要求推断系统具备\cqt{优雅降级}能力，即在卫星数据不可用时仍能保持物理合理的推断。

针对上述两个核心问题，本章提出SPIN（Spatiotemporal Physics-Guided Inference Network）模型，通过\textbf{四阶段数据融合与推断流程}系统性地加以解决：

阶段一：动态节点掩码（解决问题一）。在每个训练批次中，随机将部分监测站点标记为\cqt{目标节点}（模拟无观测状态），迫使模型学习在缺乏直接观测时的推断能力，实现归纳式空间泛化。

阶段二：本地物理特征编码（服务两个问题）。利用时间卷积网络从各节点的气象和排放时间序列中提取物理特征，为包括无观测节点在内的所有位置提供基于本地环境条件的初始浓度估计。

阶段三：物理引导的双图空间传播（解决问题一）。设计扩散核与平流核分别建模各向同性扩散和各向异性平流过程，显式解耦公式（\ref{eq:advection_diffusion}）中的扩散项与平流项，通过图传播将观测节点的信息传递至无观测区域。

阶段四：多层级约束与AOD融合（解决问题二）。设计包含推断损失、初始化损失和掩码AOD梯度损失的复合约束体系。其中AOD梯度损失创新性地将卫星数据作为空间结构约束而非强制输入，在AOD可用时提取其空间分布信息，在AOD缺失时自动回退至物理驱动模式，实现全天候推断。
...
\begin{figure}[htbp]
    ...
    \caption*{输入：观测站点历史时序$\mathbf{X}^{\text{obs}}_{t-T'+1:t}$（黑色圆点）、气象场$\mathbf{M}^{F}_{t-T'+1:t}$、排放场$\mathbf{E}^{F}_{t-T'+1:t}$、卫星AOD $\mathbf{X}^{\text{AOD}}_{t-T'+1:t}$（斜线区域表示缺失）；输出：目标站点推断$\hat{\mathbf{X}}^{\text{target}}_{t}$（蓝色曲线）及网格推断$\hat{\mathbf{X}}^{F}_t$（红色热力图）；模型：基于扩散-平流双图机制构建图结构$\mathcal{G}$，通过$\mathcal{F}_\Theta$（SPIN）建模节点间的时空相关性。}
...
（2）网格推断（Grid Inference）。对区域内所有$0.25^{\circ}$网格点进行推断，生成连续的高分辨率污染场$\hat{\mathbf{X}}^{F}_t$，以消除监测盲区。这一任务的应用场景包括：生成空间连续的污染暴露评估图、识别缺乏监测的污染热点、支撑精细化的区域联防联控决策。
...
针对第\ref{sec:infer_intro}节提出的两个核心问题，本节详细阐述SPIN（Spatiotemporal Physics-Guided Inference Network）模型的数据融合与推断流程。该模型遵循第\ref{subsec:paradigm}节提出的\cqt{物理约束的数据驱动建模}范式，将大气流体动力学中的扩散与平流机制显式地映射为图神经网络中的传播算子。SPIN与第\ref{chap:prediction}章的PCDCNet形成互补关系：PCDCNet通过LID--STD--TAD三模块解耦时间维度的化学反应、空间传输和累积过程，用于预测已知站点的未来状态；而SPIN通过扩散--平流双核解耦空间维度的各向同性与各向异性传输过程，用于推断未知位置的当前状态。

如图~\ref{fig:model_architecture}所示，SPIN的推断流程分为四个阶段：\textbf{阶段一}，通过动态节点掩码模拟数据缺失场景，赋予模型归纳式泛化能力（第\ref{subsec:stage1_masking}节）；\textbf{阶段二}，利用时间卷积网络编码本地物理特征，为所有节点提供初始浓度估计（第\ref{subsec:stage2_encoding}节）；\textbf{阶段三}，通过物理引导的双图传播机制实现空间信息交互（第\ref{subsec:stage3_propagation}节）；\textbf{阶段四}，通过多层级约束与AOD融合优化推断结果（第\ref{subsec:stage4_constraints}节）。
...
解决的问题：问题一（稀疏监测网络下的空间泛化推断）。为了使模型具备对未观测节点的推断能力，采用了动态节点掩码（Dynamic Node Masking）策略\citep{wu2021inductive}。与传统的直推式（Transductive）方法不同，本策略不依赖固定的图结构，而是通过模拟数据缺失场景，强迫模型学习通用的时空插值规律。

在每一轮训练迭代中，从所有已知观测数据的站点集合$\mathcal{V}_{\text{obs}}$中，随机采样一个子集（例如50\%）作为\cqt{掩码目标节点（Masked Target Nodes）}，记为$\mathcal{V}_{\text{mask}}$。
...
在训练过程中，模型被切断了获取这些$\mathcal{V}_{\text{mask}}$节点历史污染物数据的路径，仅能获取：（1）$\mathcal{V}_{\text{mask}}$节点本地的气象与排放特征；（2）剩余未被掩码节点（$\mathcal{V}_{\text{obs}} \setminus \mathcal{V}_{\text{mask}}$）传播过来的时空上下文信息。这种机制模拟了真实的推断场景——即推断无历史数据的盲点。更重要的是，由于每个批次的掩码是随机生成的，模型在训练过程中会\cqt{见到}同一站点作为观测节点和目标节点的不同角色。这种随机性有效防止了模型过拟合于特定站点的ID特征，迫使模型学习\cqt{基于环境背景和邻域关系进行推断}的通用物理规律。


\subsection{阶段二：本地物理特征编码}
\label{subsec:stage2_encoding}

解决的问题：服务于两个核心问题——为所有节点（包括阶段一中被掩码的目标节点）提供基于本地物理条件的初始浓度估计。
...
\subsection{阶段三：物理引导的双图空间传播}
\label{subsec:stage3_propagation}

解决的问题：问题一（稀疏监测网络下的空间泛化推断）。经过阶段二的编码后，观测节点携带真实的污染信息，而目标节点仅携带环境背景信息。本阶段通过物理引导的图传播机制，将观测节点的污染信息沿物理合理的路径传递至目标节点。
...
解决的问题：问题二（非随机缺失遥感数据的有效融合）。经过阶段三的空间传播后，每个节点的潜在表示$\mathbf{H}_i^{(L)}$已深度融合了本地物理特征和邻域污染信息。本阶段首先通过读出层将潜在表征映射为浓度值，然后通过精心设计的多层级约束体系实现AOD数据的有效融合。

\subsubsection{读出层}

经过$L$层物理引导的空间传播后，由于目标节点本身没有任何历史观测数据，无法采用自回归方式。因此设计了一个节点共享的读出层（Readout Layer），由多层感知机构成，直接将高维潜在特征映射为污染物浓度：
\begin{equation}
...
针对区域大气污染监测中的两个核心问题——稀疏监测网络下的空间泛化推断（问题一）和非随机缺失遥感数据的有效融合（问题二），本章提出了\textbf{基于物理引导的归纳式图推断网络（SPIN）}，通过四阶段数据融合与推断流程（第\ref{sec:spin_model}节）系统性地加以解决。主要贡献总结如下：
